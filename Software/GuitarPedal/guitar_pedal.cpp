...
bool isCrossFadingForward =
    true;  // True goes Source->Target, False goes Target->Source
...
static void AudioCallback(AudioHandle::InputBuffer in,
  ...
  if (effectOn != oldEffectOn) {
    ...
    if (hardware.SupportsTrueBypass() && settings.globalRelayBypassEnabled) {
      ...
      // Set the timing for when the bypass relay should trigger and when to
      // unmute.
  ...
  for (size_t i = 0; i < size; i++) {
    if (isCrossFading) {
      float crossFadeFactor = (float)samplesTilCrossFadingComplete /
                              (float)crossFaderTransitionTimeInSamples;
    ...
    if (settings.globalSplitMonoInputToStereo &&
        !settings.globalRelayBypassEnabled) {
      inputRight = inputLeft;
    }

    // Setup Master Crossfader. By default source & target is always the input
    // signal
    ...
    out[0][i] =
        crossFaderLeft.Process(crossFadeSourceLeft, crossFadeTargetLeft);
    out[1][i] =
        crossFaderRight.Process(crossFadeSourceRight, crossFadeTargetRight);
  }
  ...
  switch (m.type) {
    ...
    case ControlChange: {
      if (activeEffect != NULL) {
        ...
        int effectParamID =
            activeEffect->GetMappedParameterIDForMidiCC(p.control_number);

        if (effectParamID != -1) {
          activeEffect->SetParameterRaw(effectParamID, p.value);
          guitarPedalUI.UpdateActiveEffectParameterValue(
              effectParamID, activeEffect->GetParameterRaw(effectParamID));
        } else {
...
int main(void) {
  ...
  // Set the number of samples to use for the crossfade based on the hardware
  // sample rate
  muteOffTransitionTimeInSamples =
      hardware.GetNumberOfSamplesForTime(muteOffTransitionTimeInSeconds);
  bypassToggleTransitionTimeInSamples =
      hardware.GetNumberOfSamplesForTime(bypassToggleTransitionTimeInSeconds);
  crossFaderTransitionTimeInSamples =
  ...
  knobValueIdleTimeInSamples =
      hardware.GetNumberOfSamplesForTime(knobValueIdleTimeInSeconds);
  ...
  switchEnabledIdleTimeInSamples =
      hardware.GetNumberOfSamplesForTime(switchEnabledIdleTimeInSeconds);
      ...
      for (int i = 0; i < hardware.GetKnobCount(); i++) {
        if (knobValueCacheChanged[i]) {
          ...
          if (parameterID != -1) {
            ...
            activeEffect->SetParameterAsMagnitude(parameterID,
                                                  knobValueCache[i]);

            // Update the effect parameter on the menu system to reflect the
            // change
