...
struct ParameterMetaData {
  ...
  int valueBinCount;  // The number of distinct choices allowed for this
                      // parameter value
  const char **valueBinNames;  // The human readable display names for the bins
  int defaultValue;  // The Default Value set for this parameter the first time
                     // the device is powered up
  int knobMapping;   // The ID of the Physical Knob mapped to this Parameter. -1
                     // if this Parameter is not controlled by a Knob
  int midiCCMapping;   // The Midi CC ID mapped to this Parameter. -1 of this
                       // Parameter is not controllable via Midi CC messages
  int minValue = 0;    // The minimum value of the parameter
  int maxValue = 127;  // The maximum value of the parameter
  float fineStepSize = 0.01f;  // For Float Parameters, this will set the
...
class BaseEffectModule {
  ...
  */
  ...
  /** Sets the total number of stored presets for this effect, called by
   guitar_pedal_storage \param the number of presets for this effect.
  */
  ...
  */
  ...
  /** Gets the Type of an Effect Parameter
   \return an int representing Type of the Effect Parameter. 0 is raw u_int8_t,
   1 is Float Magnitude, 2 is Bool, 3 is Binned Int (return -1 is this is
   unknown)
  */
  int GetParameterType(int parameter_id);

  /** Gets the Bin Count of a Binned Int Effect Parameter
   \return the number of Bins for this Binned Int type Efffect Parameter or -1
   if this isn't a Binned Int type parameter
  */
  ...
  /** Gets the Bin Name of a Binned Int Effect Parameter
   ...
   \return the Names of the Bins for this Binned Int type Efffect Parameter or
   NULL if there aren't any specified
  */
  ...
  */
  ...
  /** Gets the value of an Effect Parameter as a magnitude mapped to a float
   0..1 \param parameter_id Id of the parameter to retrieve. \return the Value
   of the specified parameter mapped to a float (0..1)
  */
  ...
  */
  ...
  /** Gets the value of an Effect Parameter as a Binned Integer Value (1..Bin
   Count) \param parameter_id Id of the parameter to retrieve. \return the bin
   number as an int value (1..Bin Count) of the specified parameter
  */
  ...
  /** Gets the Parameter ID of the Effect Parameter mapped to Knob.
   ...
   \return the Parameter ID mapped to the specified knob, or -1 if no Parameter
   is Mapped to that Knob
  */
  ...
  /** Gets the Parameter ID of the Effect Parameter mapped to this Midi CC ID.
   ...
   \return the Parameter ID mapped to the specified MidiCC, or -1 if no
   Parameter is Mapped to that Midi CC
  */
  int GetMappedParameterIDForMidiCC(int midiCC_id);

  /** Sets the Raw Value for a Particular Effect Parameter.  If the Parameter ID
     isn't valid, there is no effect. \param parameter_id Id of the parameter to
     set (0 .. m_paramCount - 1). \param value Value to set on the parameter.
  */
  void SetParameterRaw(int parameter_id, uint32_t value);

  /** Sets the Value for a Particular Effect Parameter using a float magnitude
     (0..1).  If the Parameter ID isn't valid, there is no effect. \param
     parameter_id Id of the parameter to set. \param value the float magnitude
     value to set the parameter to.
  */
  void SetParameterAsMagnitude(int parameter_id, float value);

  /** Sets the Value for a Particular Effect Parameter using a bool.  If the
     Parameter ID isn't valid, there is no effect. \param parameter_id Id of the
     parameter to set. \param value the bool value to set the paramter to.
  */
  void SetParameterAsBool(int parameter_id, bool value);

  /** Sets the Value for a Particular Effect Parameter using a Binned Value.  If
     the Parameter ID isn't valid, there is no effect. \param parameter_id Id of
     the parameter to set. \param value the int value (1..Bin Count) of the bin
     to set the paramter to.
  */
  void SetParameterAsBinnedValue(int parameter_id, u_int8_t bin);

  /** Processes the Effect in Mono for a single sample.  This should only be
   called once per sample. Also, if this is called, don't call ProcessStereo
   too. \param in Input sample.
  */
  virtual void ProcessMono(float in);

  /** Processes the Effect in Stereo for a single left & right sample.  This
   should only be called once per sample. Also, if this is called, don't call
   ProcessMono too. \param inL, inR Input sample Left and Right.
  */
  virtual void ProcessStereo(float inL, float inR);

  /**  Gets the most recently calculated Sample Value for the Left Stereo
   Channel (or Mono) \return Last floating point sample for the left channel.
  */
  float GetAudioLeft();

  /** Gets the most recently calculated Sample Value for the Right Stereo
   Channel \return Last floating point sample for the right channel.
  */
  float GetAudioRight();

  /** Returns a value that can be used to drive the Effect LED brightness for a
   specific led. \return float value 0..1 for the intended LED brightness.
  */
  ...
  /** Returns the status of the Effect
   \return Value True if the Effect is Enabled and False if the Effect is
   Bypassed
  */
  bool IsEnabled();
   ...
   * @param elapsedTime a float value of how much time (in seconds) has elapsed
   ...
   */
  virtual void DrawUI(OneBitGraphicsDisplay &display, int currentIndex,
  ...
  */
  ...
  /** This function gets called when the user defines a MIDI parameter to be
     handled by callback, rather than the default implementation \param
     control_num, midicc number \param value, midi value
  */
  virtual void MidiCCValueNotification(uint8_t control_num, uint8_t value);

  /** Effect can override this and return false to disable the tap tempo
   * functionality to utilize the switch (loopers, momentary effects, etc)
 ...
 protected:
  /** Initializes the Parameter Storage and creates space for the specified
     number of stored Effect Parameters \param count  The number of stored
     parameters
  */
  ...
  const ParameterMetaData *m_paramMetaData;  // Dynamic Array of the Meta Data
                                             // for each Effect Parameter
  float m_audioLeft;  // Last Audio Sample value for the Left Stereo Channel (or
                      // Mono)
  float m_audioRight;  // Last Audio Sample value for the Right Stereo Channel
  uint32_t m_settingsArrayStartIdx;  // Start index of settings persistent
                                     // storage struct
 private:
  bool m_isEnabled;
