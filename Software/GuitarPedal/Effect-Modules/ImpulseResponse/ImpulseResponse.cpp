...
float ImpulseResponse::Process(float inputs) {
  ...
  auto input =
      Eigen::Map<const Eigen::VectorXf>(&mHistory[j], mHistoryRequired + 1);

  _AdvanceHistoryIndex(
      1);  // KAB MOD - for Daisy implementation numFrames is always 1
...
void ImpulseResponse::_SetWeights() {
  ...
  // const float gain = pow(10, -18 * 0.05) * 48000 / mSampleRate;  //KAB NOTE:
  // This made a very bad/loud sound on Daisy Seed
  for (size_t i = 0, j = irLength - 1; i < irLength; i++, j--)
    // mWeight[j] = gain * mRawAudio[i];
    mWeight[j] = mRawAudio[i];
  mHistoryRequired = irLength - 1;

  // Moved from HISTORY::EnsureHistorySize since only doing once for this module
  // (assuming same size IR's)
  //   TODO: Maybe find a more efficient method of indexing mHistory,
  //         rather than copying the end of the vector (length of IR) back to
  //         the beginning all at once.
  const size_t requiredHistoryArraySize =
      5 * mHistoryRequired;  // Just so we don't spend too much time copying
                             // back. // KAB NOTE: was 10 *
