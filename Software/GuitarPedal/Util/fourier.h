...
class Fourier {
 public:
  ...
  Fourier(void (*processor)(const T*, T*), ShyFFT<T, N, RotationPhasor>* fft,
          Wave<T>* window, size_t laps, T* in, T* middle, T* out)
      : processor(processor),
        in(in),
        middle(middle),
        out(out),
    ...
    for (size_t i = 0; i < laps * 2; i++) {
      if (writing[i]) {
        ...
        if (writepoints[i] == N) {
          ...
          process(i);   // user-defined; ought to move info from ith middle to
                        // out buffer
...
class Analyzer {
 public:
  ...
  // in, middle, out need to be arrays of size (N * laps * 2)
  Analyzer(int (*processor)(const T*), ShyFFT<T, N, RotationPhasor>* fft,
           size_t laps, T* in, T* middle)
      : processor(processor),
        in(in),
        middle(middle),
        fft(fft),
  ...
  // executes user-defined callback
  inline void process(const size_t i) { processor((middle + i * N)); }
