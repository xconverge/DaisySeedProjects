  ...
  float Process(const float input) {
    ...
    switch (state_) {
      ...
      case State::REC_FIRST:
        ...
        if (win_idx_ < kWindowSamps - 1) win_idx_ += 1;
        recsize_ = pos_;
        pos_ += inc;
    ...
    near_beginning_ =
        state_ != State::EMPTY && !Recording() && pos_ < 4800 ? true : false;
