...
class Looper
{
  public:
    ...
    float Process(const float input)
    {
        ...
        inc = state_ == State::EMPTY || state_ == State::REC_FIRST ? 1.f : GetIncrementSize();
    ...
    inline void SetHalfSpeed(bool state)
    {
        half_speed_ = state;
    }
    ...
    inline bool IsNearBeginning() const
    {
    ...
    inline size_t GetRecSize() const
    {
        return recsize_;
    }
  ...
  private:
    ...
    void InitBuff()
    {
        std::fill(&buff_[0], &buff_[buffer_size_ - 1], 0);
    }

    /** Get a floating point sample from the buffer */
    inline const float Read(size_t pos) const
    {
        return buff_[pos];
    }
    ...
    inline void Write(size_t pos, float val)
    {
        buff_[pos] = val;
    }

    /** Linear to Constpower approximation for windowing*/
    float WindowVal(float in)
    {
