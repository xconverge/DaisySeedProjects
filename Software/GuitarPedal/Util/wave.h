...
class Wave {
 public:
  ...
  Wave(std::function<T(T)> shape, T left = 0, T right = 1,
       bool periodic = true) {
    this->shape = shape;
    this->left = left;
    this->right = right;
...
#ifdef FUNCTIONAL
  T lookup(T input) { return shape(input); }
#else
  T lookup(T input) {
  ...
  T operator()(T phase) { return lookup(phase); }
 ...
 private:
  ...
  T endpoint;  // if (this->periodic == false), provides a value for
               // (*this)(right)

  std::function<T(T)> shape;

  T none(int center) { return table[center]; }
