...
uint32_t ShiftSettingsToMatchCurrentParameters(uint32_t prev_params, uint32_t curr_params, uint32_t effectID,
                                               uint32_t presetsCount, uint32_t shiftStartIdx, uint32_t currentMaxIdx)
{
...
void LoadPresetFromPersistentStorage(uint32_t effectID, uint32_t presetID)
{
    ...
    if (effectID >= 0 && effectID < (uint32_t)availableEffectsCount && presetID < presetCount)
    {
        ...
        for (int paramID = 0; paramID < paramCount; paramID++)
        {
            ...
            {
                availableEffects[effectID]->SetParameterRaw(paramID,
                                                            settings.globalEffectsSettings[startIdx + paramID]);
            }
        }
    }
}
...
void LoadEffectSettingsFromPersistantStorage()
{
    ...
    for (int effectID = 0; effectID < availableEffectsCount; effectID++)
    {
        ...
        else if (prevParamCount < paramCount)
        {
            ...
            globalEffectsMaxIdx = ShiftSettingsToMatchCurrentParameters(
                prevParamCount, paramCount, effectID, presetsCount, globalEffectsSettingMemIdx, globalEffectsMaxIdx);
    ...
    if (effectID >= 0 && effectID < availableEffectsCount)
    {
        ...
        if (canWriteNewPreset)
        {
            for (int paramID = 0; paramID < paramCount; paramID++)
            {
                ...
                {
                    SetSettingsParameterValueForEffect(effectID, paramID,
                                                       availableEffects[effectID]->GetParameterRaw(paramID), startIdx);
                }
            }
            // Set the new maximum
