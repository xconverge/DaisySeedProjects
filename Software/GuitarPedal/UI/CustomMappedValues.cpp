...
MyMappedFloatValue::MyMappedFloatValue(float min, float max, float defaultValue, Mapping mapping, const char *unitStr,
                                       uint8_t numDecimals, bool forceSign, float coarseStepSize0to1_,
                                       float fineStepSize0to1_)
    : value_(defaultValue), min_(min), max_(max), default_(defaultValue), mapping_(mapping), unitStr_(unitStr),
      numDecimals_(numDecimals), forceSign_(forceSign), coarseStepSize0to1_(coarseStepSize0to1_),
...
void MyMappedFloatValue::Step(int16_t numSteps, bool useCoarseStepSize)
{
    ...
    const auto step = numSteps * (useCoarseStepSize ? coarseStepSize0to1_ : fineStepSize0to1_);
